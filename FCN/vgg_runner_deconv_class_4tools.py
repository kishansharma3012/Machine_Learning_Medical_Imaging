...
def main(_):
    ...
    # CLASSIFICATION
    y_class = tf.placeholder(tf.float32, [None, FLAGS.nb_classes], name='y-class-input')

    y_conv_class = vgg.vgg_only_conv(x, is_training=False, verbose=True, classif = True)
    y_conv_class = vgg.vgg_finish_conv(y_conv_class, num_classes=FLAGS.nb_classes, is_training=False, verbose=True)

    with tf.name_scope('loss_class'):
        cross_entropy = tf.reduce_mean(tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_class,
                                                                               logits=y_conv_class),1))
    tf.summary.scalar('loss_classification', cross_entropy)

    with tf.name_scope('Adam_optimizer_class'):
        train_step_class = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)

    with tf.name_scope('accuracy_class'):
        with tf.name_scope('correct_prediction_class'):
            correct_prediction_class = tf.equal(tf.argmax(y_conv_class, 1), tf.argmax(y_class, 1))
        with tf.name_scope('accuracy_class'):
            correct_prediction_class = tf.cast(correct_prediction_class, tf.float32)
    accuracy_class = tf.reduce_mean(correct_prediction_class)
    tf.summary.scalar('accuracy_classification', accuracy_class)
    '''
    ...
    y_gaussian = tf.placeholder(tf.float32, [None, 12, FLAGS.width, FLAGS.height], name='y-input')
    y_gaussian = tf.transpose(y_gaussian, [0, 2, 3, 1])
    y_gaussian = tf.reshape(y_gaussian, [-1, FLAGS.width, FLAGS.height, 12]) #N, Width, height, Ins.*Joints
    y_gaussian = tf.transpose(y_gaussian, [0, 3, 1, 2]) #N, Ins.*Joints, Width, height
    y_gaussian_reshape = tf.expand_dims(y_gaussian, -1) #N, Ins.*Joints, Width, height, 1
    ...
    add_imgs_to_summary(y_gaussian_reshape, range(12), base_name="gt/img")
    # build network
    y_conv_class = vgg.vgg_only_conv(x, is_training=False, verbose=False)
    # model for localization
    y_conv = vgg.vgg_only_deconv(y_conv_class, verbose=False)
    y_conv = normalize_tensor(y_conv)
    y_conv_sum, y_conv_reshape = produce_sum_maps(y_conv, FLAGS)
    prediction_bb = get_images_bb_max_xy(y_conv_reshape, gaussian_idx_list=range(12), k=5, args=FLAGS)
    add_imgs_to_summary(prediction_bb, range(12), base_name="prediction/bb")

    # model for classification
    y_conv_class = vgg.vgg_finish_conv(y_conv_class, num_classes=FLAGS.nb_classes, is_training=False, verbose=False)


    #LOCALIZATION PART
    with tf.name_scope('accuracy'):
        with tf.name_scope("prediction"):
            correct_prediction = TrainerAll.accuracy_function(labels=y_gaussian,
                                                       prediction=tf.reshape(y_conv_reshape, [-1, 12, FLAGS.width, FLAGS.height]),
                                                       acc_name='locali',
                                                       threshold = 5,
                                                       transpose=False,
                                                       verbose=False,
        ...
        with tf.name_scope("accuracy"):
            accuracy = tf.reduce_mean(correct_prediction)
            #accuracy_10 = tf.reduce_mean(correct_prediction_10)
    tf.summary.scalar('accuracy_localization_15', accuracy)
    #tf.summary.scalar('accuracy_localization_10', accuracy_10)

    with tf.name_scope("loss"):
        loss = TrainerAll.loss_function(labels=y_gaussian_reshape,
                                 prediction=tf.reshape(y_conv_reshape, [-1, 12, FLAGS.width, FLAGS.height]),
                                 cost_name="softmax_cross_entropy",
                                 transpose=False,
                                 args=FLAGS)
    ...
    with tf.name_scope('accuracy_class'):
        with tf.name_scope('correct_prediction_class'):
            correct_prediction_class = tf.equal(tf.argmax(y_conv_class, 1), tf.argmax(y_class, 1))
        with tf.name_scope('corr_prediction_class'):
            correct_prediction_class = tf.cast(correct_prediction_class, tf.float32)
    accuracy_class = tf.reduce_mean(correct_prediction_class)
    tf.summary.scalar('accuracy_classification', accuracy_class)
    ...
    alpha_loc = 1.0
    alpha_class = 100.0
    with tf.name_scope('added_loss'):
        loss_class_and_loc = tf.add(tf.multiply(alpha_class,cross_entropy),tf.multiply(alpha_loc,loss))

    with tf.name_scope('Adam_optimizer_class_and_loc'):
        train_step_class_and_loc = tf.train.AdamOptimizer(1e-4).minimize(loss_class_and_loc)

    tf.summary.scalar('loss_class_and_loc', loss_class_and_loc)
    '''
    ...
    checkpoint_path = '../MLMI_FCN/data/vgg_16.ckpt'
    ...
    dataset_batch = Batch(file_path=FLAGS.data_dir_train, validation_perc=0.05,
                          test_perc=0.01)  # , test_perc=0.9)  # , test_perc=0.60)
    ...
    Trainer(dataset_batch=dataset_batch,
            ...
            ).train_by_epoch(FLAGS.epochs, save_checkpoint=True, classification=True)
    ...
    Trainer(dataset_batch=dataset_batch,
            ...
            sess=sess, train_name=FLAGS.train_name, initialize_variables=False
            ).train_by_epoch(FLAGS.epochs, save_checkpoint=True, classification=False)
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_name', type=str,
                        default='_4tools_10joints_30_ep_class__attempt2' + strftime("%Y_%m_%d_%H_%M_%S", gmtime()))
    parser.add_argument('--data_dir_train', type=str,
                        default='./data/train_images_224_4tools_10joints_10ptc_shuffled_chuncked.hdf5')
    parser.add_argument('--data_dir_test', type=str,
                        default='./data/test_images_224_4tools_10joints_chuncked.hdf5')
