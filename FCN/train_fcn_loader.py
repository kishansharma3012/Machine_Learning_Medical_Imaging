...
class Trainer(object):
    ...
    def train_by_epoch(self, nb_epochs, save_checkpoint=None, classification = False):
        ...
        iteration = 0
        train_writer = tf.summary.FileWriter('./final_results/train_' + self.train_name, self.sess.graph)
        validation_writer = tf.summary.FileWriter('./final_results/validation_' + self.train_name)
        test_writer = tf.summary.FileWriter('./final_results/test_' + self.train_name)
        ...
        while self.dataset_batch.current_epoch < nb_epochs:
            ...
            _, acc_train = self.sess.run([merged_summary, self.model_eval],
                                        feed_dict=self.feed_dict(train=True, nexBatch=True,classification=classification))
