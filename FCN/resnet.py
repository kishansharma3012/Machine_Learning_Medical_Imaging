...
def resnet_only_deconv(inputs,
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d]):
        ...
        ## DECODING ##
        ##############
        net = slim.conv2d(inputs, 2048, [1, 1], activation_fn=tf.nn.relu, normalizer_fn=None, scope='deconv1')
        if verbose: print(net.shape, 'deconv1')

        net = upsample_fn(net, 1024, [2,2], scope="other_up1")
        net = tf.reshape(net, [tf.shape(net)[0], 14, 14, 1024])
  ...
  with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d, bottleneck,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        if num_classes:
          net = slim.conv2d(net, num_classes, [1, 1], activation_fn=tf.nn.relu,
...
def resnet_v1_50(inputs,
  ...
  return resnet_v1(inputs, blocks, num_classes, is_training,
                   ...
                   reuse=reuse, scope=scope, verbose = verbose)
