...
from train_fcn_resnet import Trainer
...
def main(_):
    ...
    with slim.arg_scope(resnet.resnet_arg_scope()):
        y_conv = resnet.resnet_v1_50(x, global_pool=True)
        y_conv = resnet.resnet_only_deconv(y_conv, verbose=True, args = FLAGS)  # outputs: [-1, 224, 224, 5, 2]
    ...
    #tf.summary.scalar('accuracy_localization_15_left', accuracy_left)
    #tf.summary.scalar('accuracy_localization_15_right', accuracy_right)
    ...
    with tf.name_scope('adam_optimizer'):
        ...
        optimizer = tf.train.MomentumOptimizer(momentum=0.9, learning_rate=1e-1)
    ...
    variables_to_restore = slim.get_variables_to_restore(
        exclude=[ '.*[Aa]dam.*', "conv_up.*","up.*_conv.*", ".*Momentum.*",
                 "variable.*", "save/RestoreV2.*",".*up.*",".*deconv.*"])
    ...
    checkpoint_path = '../MLMI_FCN/data/resnet_v1_50.ckpt'
    ...
    Trainer(dataset_batch=dataset_batch,
            model_eval=accuracy, 
            train_step=train_step_loc, tf_data_input=x,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_name', type=str,
                        default='_resnet_full_data_50ep-test-e-2-neg-momentum-global-pool-final' + strftime("%Y_%m_%d_%H_%M_%S", gmtime()))
    parser.add_argument('--data_dir_train', type=str,
                        default='./data/train_class+loc_images_224_chuncked.hdf5')
    parser.add_argument('--data_dir_test', type=str,
                        default='./data/test_class+loc_images_224_chuncked.hdf5')
