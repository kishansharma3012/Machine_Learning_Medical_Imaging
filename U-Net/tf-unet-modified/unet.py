...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    #weights.append(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'fc1'))
    #biases.append(tf.get_variable('fc1/biase'))
    # fully connected layer 2 of 256
    fc2 = tf.layers.dense(fc1, 256, name='fc2')
    #weights.append(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'fc2'))
    #biases.append(tf.get_variable('fc2/biase'))
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", lambda_c = 1,lambda_l = 1, cost_kwargs={}, **kwargs):
        ...
        #lambda_c = self.locali_cost/(self.classi_cost + self.locali_cost)
        #lambda_l = self.classi_cost/(self.classi_cost + self.locali_cost)
        lambda_c = 60.0/61.0
        lambda_l = 1.0/61.0
        ...
        #self.total_cost = (self.t_cost/(2.0*self.classi_cost))*self.classi_cost + (self.t_cost/(2.0*self.locali_cost))*self.locali_cost 
