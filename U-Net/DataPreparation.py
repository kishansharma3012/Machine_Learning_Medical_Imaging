...
#### 2. Resize the frames to 350*350 from original size of 576*720.
...
class DataPreparation:
    ...
    def fill_label_list(self, lis, joint_dict):
        if joint_dict["class"] == "LeftClasperPoint":
            ...
            lis[10] = float(joint_dict["x"])
            lis[11] = float(joint_dict["y"])
        else:
            print("error wrong joint passed")
    ...
    def append_frame_and_label(self, frame, anno, train_store, label_store, flip_hor=False):
        ...
        #Append label
        tool1 = [-1]*12 #Right clasper instrument joints: series of x,y,x,y --> LeftClasperPointx, LeftClasperPointy, RightClasperPointx, RightClasperPointy,
                          #ShaftPointx, ShaftPointy, TrackedPointx, TrackedPointy, EndPointx, EndPointy, HeadPointx, HeadPointy
        tool2 = [-1]*12 #Left clasper instrument joints
        tool3 = [-1]*12 #Right scissor instrument joints
        tool4 = [-1]*12 #left scissor instrument joints
    ...
    def extractLocaliLabels(self):
        ...
        locali_label_store = self.hdf5_file.create_earray(self.hdf5_file.root, 'localilabels', tables.Float32Atom(), shape=(0,12,350,350))
        imcount = 1
        for image_label in self.tempfile.root.rawlabels:
            p_maps = p_map_generator(image_label, image_size = (350,350))
...
if __name__ == "__main__":
    ...
    train_output_file_name = "train_images_350.hdf5"
    ...
    test_output_file_name = "test_images_350.hdf5"
