...
from tf_unet_modified.layers import (weight_variable, weight_variable_devonc, bias_variable, 
                            ...
                            cross_entropy, sigmoid_likelihood, localization_prediction)
...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    for layer in range(0, layers): # we want an extra convoluton of 2048 at the end of contraction path
        ...
        #epsilon = 1e-3
        #BN = tf.nn.batch_normalization(conv1,batch_mean,batch_var,None, None,epsilon)
    ...
    fc1 = tf.layers.dense(tf.contrib.layers.flatten(tmp_h_conv), 512)
    ...
    # Output Map
    weight = weight_variable([1, 1, features_root, 12], stddev)# 12 = 2_tools*6_joints_per_tool
    bias = bias_variable([12])
    ...
    return variables, output_map, int(in_size - size)
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.locali_y = tf.placeholder("float", shape=[None, 12, 224,224]) # For localization maps
        self.keep_prob = tf.placeholder(tf.float32) #dropout (keep probability)

        self.variables, locali_map, self.offset = create_conv_net(self.x, self.keep_prob, channels, n_class, **kwargs)
        
        #self.classi_cost = self._get_cost(logits, cost, cost_kwargs)
        self.locali_cost = self._get_cost(locali_map, "localization_cost", cost_kwargs)
        
        # For classification
        #self.classi_predicter = sigmoid_likelihood(logits)
    ...
    def get_locali_accuracy(self, prediction_x, prediction_y):
        ...
        x_l = tf.arg_max(self.locali_labels, 2)
        y_l = tf.arg_max(self.locali_labels, 3)
        ...
        f = tf.less(final,self.threshold)
        return tf.reduce_mean(tf.cast(f, 'float'))
    ...
    def get_locali_error(self, prediction_x, prediction_y):
        ...
        final_x = tf.square(tf.subtract(x, x_l))
        final_y = tf.square(tf.subtract(y, y_l))
        final = tf.reduce_mean(tf.sqrt(tf.add(final_x, final_y)))
        return final
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        if cost_name == "sigmoid_cross_entropy":
            # Ibrar: Modification to define our loss
            tool_wise_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels = self.classi_y)
            sample_wise_loss = tf.reduce_sum(component_wise_loss, 1)
            ...
            resh_logits = tf.reshape(t_logits, [-1,12, 224*224])
            resh_labels = tf.reshape(t_labels, [-1,12,224*224])

            # Now for each sample, we have 12 joints, and for each joint 224*224 classes
            loss = tf.reduce_mean(tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=resh_logits, labels=resh_labels),1))
    ...
    def locali_predict(self, model_path, x_test):
        ...
        with tf.Session() as sess:
            ...
            y_dummy = np.empty((x_test.shape[0], 224,224,6,2))
            
            prediction = sess.run(self.locali_predicter, feed_dict={self.x: x_test, self.locali_y: y_dummy, self.keep_prob: 1.})
            
        return prediction # prediction is N*12 x position, N*12 y positions
